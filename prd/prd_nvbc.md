...
* **JSON Output Specification:**
  ...
  {  
      ...
      "api_version":"ambassador.2026.01.01"
  }
  ```
...
#### **Function: get_nvbc_winners**

* **Loại:** READ
* **Mục đích:** Truy xuất bảng xếp hạng (Leaderboard) các dược sĩ đạt giải trong các sự kiện thi đua. Hàm tự động phân tích loại sự kiện (tháng/quý), lấy thông tin dược sĩ và tính toán điểm tích lũy trong khoảng thời gian tương ứng để xếp hạng.
* **Logic Xử lý:**
    1.  **Parse Event & Timeframe:** * Từ chuỗi `reward_event` đầu vào (VD: `01_26_th_monthly_reward`), xác định tháng hiện tại (`start_date`: 01/01/2026, `end_date`: 01/02/2026).
        * Từ tháng đó, tự động suy ra chu kỳ Quý (`start_date_quarter` và `end_date_quarter`) và tự động truy vấn tìm mã sự kiện Quý (VD: `q12026_product_expert_reward`) từ cấu hình hệ thống (`nvbc_reward_type`).
    2.  **Filter & Enrich:** Lấy danh sách SĐT trúng thưởng của cả sự kiện Tháng và sự kiện Quý từ `nvbc_reward_list`. Sau đó `JOIN` với `f_crawl_activate_ecom` (để lấy tên dược sĩ) và `d_master_khachhang` (để lấy tên tỉnh/thành phố).
    3.  **Ranking (Tính điểm & Xếp hạng):**
        * **Giải Tháng (`th_monthly_winners`):** Tính tổng điểm tích luỹ (điểm xem bài `nvbc_track_view`, điểm giới thiệu `nvbc_ref_month`, điểm điểm danh `nvbc_streak_daily`) trong **tháng** diễn ra sự kiện. Xếp hạng bằng hàm `RANK()`.
        * **Giải Quý (`product_expert_winners`):** Tính tổng điểm tích lũy trong **tháng** diễn ra sự kiện (giống giải tháng) VÀ tính thêm điểm duy trì (tổng điểm trong toàn bộ **quý**). Xếp hạng dựa trên tổng điểm quý và giới hạn top 50 người.
...
```json
{
    ...
    "meta_info": {
        ...
        "expert_event": "q12026_product_expert_reward",
        "quarter_from": "2026-01-01",
        "quarter_to": "2026-04-01"
    },
    "th_monthly_winners": [
        {
            "phone": "0909xxxxxx",
    ...
    "product_expert_winners": [
        {
            "phone": "0988xxxxxx",
            "ten_duoc_si": "Lê Văn C",
            "ten_tinh": "Đà Nẵng",
            "tong_diem_tich_luy": 120,
            "tong_diem_duy_tri": 450,
            "rank_theo_diem": 1
        }
    ]
}
