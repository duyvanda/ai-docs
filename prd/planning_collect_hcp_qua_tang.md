...
* **[Mục tiêu 1]: Quản lý và kiểm soát số lượng cấp phát chặt chẽ:** Đảm bảo tổng số lượng quà tặng không vượt quá định mức tối đa cho một HCP, định mức cá nhân của CRS và tổng số lượng của khu vực (CRM).
...
1.  **Xem thông tin chương trình:** 
    *   Khi truy cập màn hình, CRS sẽ thấy rõ các thông tin về đợt tặng quà hiện tại: Tên chương trình, thời gian đăng ký (mở/đóng), số lượng tối đa cho phép trên mỗi HCP, loại kênh và đối tượng áp dụng.
...
4.  **Gửi đề xuất:**
    ...
    *   Hệ thống tự động kiểm tra ngay các quy định kinh doanh:
        ...
        *   Tổng số lượng quà tặng có vượt mức cho phép của 1 HCP không?
        *   Tổng số lượng của CRS có vượt định mức cá nhân không?
        *   Tổng số lượng của cả Team có vượt định mức vùng (CRM) không?
5.  **Kết quả hiển thị:**
...
| `dinh_muc_toi_da_hcp` | integer | Tổng số lượng tối đa cho phép trên 1 HCP |
...
**Table 3: `planning_collect_hcp_qua_tang_dinh_muc_crm`** (Bảng Định mức Số lượng)
Quản lý định mức số lượng cho CRS và CRM theo từng chương trình và từng vật tư.
...
| `dinh_muc` | integer | Số lượng tối đa được phép cấp phát cho vật tư này |
...
* **JSON Output Specification:**
    ```json
    {
        ...
        "quy_tac": "Tên chương trình: ... \n Định mức số lượng tối đa HCP: 5 ...",
        "lst_hcp": [
            {
                "ma_crs": "MR1234",
                "ma_hcp_2": "HCP001",
...
* **JSON Output:**
    * **Thành công:**
        ```json
        { "status": "ok", "success_message": "Đã nhận thành công, <Tổng số lượng đã nhập>: 4" }
        ```
    * **Thất bại:**
        ```json
...
### 6.4. Nhóm Xóa Đề xuất (CRS)

#### **Function:** `delete_planning_collect_hcp_qua_tang`
...
* **JSON Output:**
    * **Thành công:**
        ```json
        { "status": "ok", "success_message": "Xóa thành công!" }
        ```
    * **Thất bại:**
        ```json
        { "status": "fail", "error_message": "[Nội dung lỗi]" }
